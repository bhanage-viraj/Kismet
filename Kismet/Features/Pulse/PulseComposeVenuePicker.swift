...
/// Horizontal venue cards — matches Pulse compose mockup (green selected + shadow cards).
struct VenuePickerChipRow: View {
	...
	private func venueCard(_ venue: GroundedVenue) -> some View {
		...
		let accent = KismetTheme.Status.free
		...
		Button {
			...
			VStack(alignment: .leading, spacing: 0) {
				HStack(alignment: .top) {
					Image(systemName: "mappin.and.ellipse")
						.font(.caption.weight(.semibold))
						.foregroundStyle(isSelected ? Color.white.opacity(0.95) : .primary)
			...
			.background {
				RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
					.fill(cardFill(isSelected: isSelected, accent: accent))
					.shadow(
						color: isSelected
							? accent.opacity(0.35)
	...
	private func cardFill(isSelected: Bool, accent: Color) -> AnyShapeStyle {
		if isSelected {
		...
		return AnyShapeStyle(colorScheme == .dark ? Color.white.opacity(0.12) : Color.white)
	}
