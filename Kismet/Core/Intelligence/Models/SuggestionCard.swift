...
struct GroundedActivityCandidates: Sendable, Equatable, Hashable {
	var suggested: PulseActivity
	var alternatives: [PulseActivity]

	var all: [PulseActivity] {
		[suggested] + alternatives.filter { $0 != suggested }
	}
}
...
struct SuggestionCard: Identifiable, Sendable {
	...
	/// Suggested Pulse activity chip plus alternatives for compose.
