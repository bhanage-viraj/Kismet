...
enum VenueQueryType: String, Sendable, Hashable, CaseIterable {
	...
	/// Map Pulse compose activity chips onto MapKit search bias.
	static func from(pulseActivity: PulseActivity) -> VenueQueryType {
		switch pulseActivity {
		case .coffee: .coffee
		case .walk: .park
		case .badminton: .other
		case .movies: .other
		case .more: .cafe
		}
	}

	/// Natural-language MapKit query for a compose activity.
	static func searchQuery(for activity: PulseActivity) -> String {
		switch activity {
		case .coffee: "coffee"
		case .walk: "park"
		case .badminton: "badminton sports"
		case .movies: "cinema movie theater"
		case .more: "cafe"
		}
	}
}
...
struct VenueQuery: Sendable {
	...
	/// Overrides `type.naturalLanguageQuery` for MapKit search (e.g. badminton / movies).
	var naturalLanguageOverride: String? = nil

	var searchText: String {
		naturalLanguageOverride ?? type.naturalLanguageQuery
	}
	...
	init(
		...
		naturalLanguageOverride: String? = nil
	) {
		self.type = type
		self.origin = origin
		self.transportPreference = transportPreference
		self.naturalLanguageOverride = naturalLanguageOverride
	}
}
